...
{
	size_t margin_size = _pattern_size + _search_wnd_size;
	...
	{
		auto start_point = cv::Point2i(ap.x - _pattern_size / 2, ap.y - _pattern_size / 2);
...
cv::Point2i GoalPostsLocator::_match_pattern(cv::Mat pattern, cv::Point2i sp)
{
	...
	for(int y=sp.y-_search_wnd_size; y<sp.y+_search_wnd_size+1; ++y)
	{
		for(int x=sp.x-_search_wnd_size; x<sp.x+_search_wnd_size+1; ++x)
		{
...
float GoalPostsLocator::_calc_metric(int x, int y, cv::Mat pattern, cv::Point2i sp)
{
	...
	int stride = width - _pattern_size;

	for(int i=0; i<_pattern_size; ++i)
	{
		for(int j=0; j<_pattern_size; ++j)
		{
			acc[*pptr == 0] += *iptr;
			cnt[*pptr == 0] += 1;
			++pptr;
